      ...
      <div className="cn-header">
        <div className="cn-header-left">
          <div>
            ...
            <p>
              Patient: <strong>{selectedChild.name}</strong>
            </p>
          </div>
        </div>
        ...
        <div className="cn-forms">
          {activeMode === "note" && (
            <div className="cn-form-card">
              <h3>
                <i className="ri-sticky-note-line" /> Add Doctor Note
              </h3>
              <form onSubmit={handleNoteSubmit}>
                ...
                <div className="cn-field">
                  <label>
                    Note <span className="cn-required">*</span>
                  </label>
                  <textarea
                    name="notes"
                    rows={4}
                ...
                {noteMsg && (
                  <p
                    className={
                      noteMsg.startsWith("✓") ? "cn-success" : "cn-error"
                    }
                  >
                    {noteMsg}
                  </p>
          ...
          {activeMode === "prescription" && (
            <div className="cn-form-card">
              <h3>
                <i className="ri-capsule-line" /> Prescribe Medicine
              </h3>
              <form onSubmit={handleMedSubmit}>
                <div className="cn-field">
                  <label>
                    Medicine Name <span className="cn-required">*</span>
                  </label>
                  <input
                    type="text"
                ...
                <div className="cn-field">
                  ...
                  <input
                    ...
                    value={medForm.medication_dosage}
                    onChange={handleMedChange}
                  />
                </div>
                <div className="cn-field">
                ...
                {medMsg && (
                  <p
                    className={
                      medMsg.startsWith("✓") ? "cn-success" : "cn-error"
                    }
                  >
                    {medMsg}
                  </p>
                ...
                <button
                  type="submit"
                  className="cn-btn cn-btn-rx"
                  disabled={savingMed}
                >
                  <i className="ri-save-line" />{" "}
                  {savingMed ? "Saving..." : "Save Prescription"}
                </button>
              </form>
            </div>
          )}
            ...
            {historyRecords.length === 0 ? (
              ...
              historyRecords.map((record, i) => {
                ...
                return (
                  <div
                    key={i}
                    className={`cn-record-card ${getTypeClass(record.record_type)}`}
                  >
                    <div className="cn-record-top">
                      <span className="cn-record-type-badge">
                        {record.record_type}
                      </span>
                      <span className="cn-record-date">
                        {formatDate(record.record_date)}
                      </span>
                      {isLongTerm && (
                        <span className="cn-longterm">Long-term</span>
                      )}
                    </div>

                    {record.title && (
                      <strong className="cn-record-title">
                        {record.title}
                      </strong>
                    )}
                    {record.doctor_name && (
                      <span className="cn-record-doctor">
                        {record.doctor_name}
                      </span>
                    )}
                    {record.medication_name && (
                      <div className="cn-med-row">
                        <span className="cn-med-name">
                          {record.medication_name}
                        </span>
                        {record.medication_dosage && (
                          <span className="cn-med-dot">•</span>
                        )}
                        {record.medication_dosage && (
                          <span className="cn-med-dosage">
                            {record.medication_dosage}
                          </span>
                        )}
                        {record.treatment && (
                    ...
                    {record.diagnosis && (
                      <p className="cn-record-text">
                        <span className="cn-record-label">Diagnosis:</span>{" "}
                        {record.diagnosis}
                      </p>
                    )}
                    {record.treatment && !record.medication_name && (
                      <p className="cn-record-text">
                        <span className="cn-record-label">Treatment:</span>{" "}
                        {record.treatment}
                      </p>
                    )}
                    {cleanNotes && (
