...
function ViewField({ label, value, fullWidth }) {
  return (
    <div className="dp-field" style={fullWidth ? { gridColumn: "1 / -1" } : {}}>
      <label style={{ fontSize: 12, color: "#6b7280", marginBottom: 2 }}>
        {label}
      </label>
      <div className="dp-view-value">
  ...
  return (
    <div className="dp-tag-input">
      ...
      <div className="dp-tag-row">
        ...
        />
        <button className="dp-tag-add" onClick={add}>
          Add
        </button>
      </div>
    </div>
...
function TagDisplay({ tags }) {
  ...
  return (
    <div className="dp-tags" style={{ marginTop: 4 }}>
      {tags.map((t, i) => (
        <span key={i} className="dp-tag" style={{ cursor: "default" }}>
          {t}
        </span>
      ))}
    </div>
  ...
  return (
    ...
    >
      ...
      {status.type === "error" && status.message && (
        <span style={{ fontSize: 13, marginLeft: 8 }}>{status.message}</span>
      )}
    </div>
  );
}
...
export default function DoctorProfile() {
  ...
  const loadProfile = () => {
    ...
    fetch(`${API_BASE}/doctor-profile`, {
      ...
      .then((data) => {
        ...
        if (data.availability && data.availability.length === 7)
  ...
  const handleSave = async () => {
    ...
    const payload = {
      ...
      basic,
      professional,
      qualifications: stripId(qualifications),
      experience: stripId(experience),
      certifications: stripId(certifications),
      languages,
      expertise,
      publications: stripId(publications),
      availability,
      emergency,
    };
  ...
  const addQual = () =>
    setQualifications((q) => [
      ...q,
      { id: uid(), degree: "", institution: "", year: "", country: "" },
    ]);
  const updateQual = (id, k, v) =>
    setQualifications((q) =>
      q.map((x) => (x.id === id ? { ...x, [k]: v } : x)),
    );
  const removeQual = (id) =>
  ...
  const addExp = () =>
    setExperience((e) => [
      ...e,
      { id: uid(), role: "", hospital: "", from: "", to: "", current: false },
    ]);
  const updateExp = (id, k, v) =>
    setExperience((e) => e.map((x) => (x.id === id ? { ...x, [k]: v } : x)));
  ...
  const updateCert = (id, k, v) =>
    setCertifications((c) =>
      c.map((x) => (x.id === id ? { ...x, [k]: v } : x)),
    );
  const removeCert = (id) =>
  ...
  const addPub = () =>
    setPublications((p) => [
      ...p,
      { id: uid(), title: "", journal: "", year: "" },
    ]);
  const updatePub = (id, k, v) =>
    setPublications((p) => p.map((x) => (x.id === id ? { ...x, [k]: v } : x)));
  const removePub = (id) =>
    setPublications((p) => p.filter((x) => x.id !== id));

  const toggleDay = (idx) =>
    setAvailability((a) =>
      a.map((d, i) => (i === idx ? { ...d, available: !d.available } : d)),
    );
  const updateDay = (idx, k, v) =>
    setAvailability((a) => a.map((d, i) => (i === idx ? { ...d, [k]: v } : d)));

  const initials =
    `${basic.firstName?.[0] || ""}${basic.lastName?.[0] || ""}` || "DR";
    ...
    <div className="dp-page">
      <div className="dp-hero">
        ...
        <div style={{ display: "flex", flexDirection: "column", gap: 8 }}>
          {!editing ? (
            <button className="dp-save-btn" onClick={handleEdit}>
              Edit Profile
            </button>
          ) : (
            <>
      ...
      <div className="dp-body">
          ...
          {editing ? (
            <div className="dp-grid-3">
              ...
              <Field label="Sub-Specialty">
                <input
                  value={professional.subSpecialty}
              ...
              <Field label="SLMC Registration No.">
                <input
                  value={professional.slmcNumber}
                  placeholder="SLMC-XXXXX"
                  onChange={(e) =>
                    setProfessional({
                      ...professional,
                      slmcNumber: e.target.value,
                    })
                  }
                />
              </Field>
        ...
        <section className="dp-card">
          ...
          {editing ? (
            <>
              <p className="dp-hint">
                Add your degrees — MBBS, MD, MS, Fellowship, etc.
              </p>
                  ...
                  <div className="dp-entry-fields dp-grid-2">
                    <Field label="Degree / Qualification">
                      <input
                        ...
                        onChange={(e) =>
                          updateQual(q.id, "degree", e.target.value)
                        }
                      />
                    </Field>
                    <Field label="Institution / University">
                      <input
                        ...
                        onChange={(e) =>
                          updateQual(q.id, "institution", e.target.value)
                        }
                      />
                    </Field>
                    <Field label="Year of Completion">
                      <input
                        ...
                        onChange={(e) =>
                          updateQual(q.id, "year", e.target.value)
                        }
                      />
                    </Field>
                    <Field label="Country">
                      <input
                        ...
                        onChange={(e) =>
                          updateQual(q.id, "country", e.target.value)
                        }
                      />
                    </Field>
                  </div>
        ...
        <section className="dp-card">
          ...
          {editing ? (
            <>
              <p className="dp-hint">
                List your hospital roles, residencies, fellowships, and past
                positions.
              </p>
                  ...
                  <div className="dp-entry-fields">
                    <div className="dp-grid-2">
                      <Field label="Role / Position">
                        <input
                          ...
                          onChange={(ev) =>
                            updateExp(e.id, "role", ev.target.value)
                          }
                        />
                      </Field>
                      <Field label="Hospital / Institution">
                        <input
                          value={e.hospital}
                          placeholder="e.g. Lady Ridgeway Hospital"
                          onChange={(ev) =>
                            updateExp(e.id, "hospital", ev.target.value)
                          }
                        />
                      </Field>
                      <Field label="From">
                        <input
                          ...
                          value={e.from}
                          onChange={(ev) =>
                            updateExp(e.id, "from", ev.target.value)
                          }
                        />
                      </Field>
                      <Field label="To">
                        <input
                          ...
                          disabled={e.current}
                          onChange={(ev) =>
                            updateExp(e.id, "to", ev.target.value)
                          }
                        />
                      </Field>
                    </div>
                    <label className="dp-check-label">
                      <input
                        ...
                        checked={e.current}
                        onChange={(ev) =>
                          updateExp(e.id, "current", ev.target.checked)
                        }
                      />
        ...
        <section className="dp-card">
          ...
          {editing ? (
            <>
              <p className="dp-hint">
                Include board certifications, professional memberships, and
                  ...
                  <div className="dp-entry-fields dp-grid-2">
                    <Field label="Certification Name">
                      <input
                        ...
                        onChange={(e) =>
                          updateCert(c.id, "name", e.target.value)
                        }
                      />
                    </Field>
                    <Field label="Issuing Body">
                      <input
                        value={c.issuingBody}
                        placeholder="e.g. Sri Lanka College of Pediatricians"
                        onChange={(e) =>
                          updateCert(c.id, "issuingBody", e.target.value)
                        }
                      />
                    </Field>
        ...
        <section className="dp-card">
          ...
          {editing ? (
            <>
              <p className="dp-hint">
                Add conditions treated, procedures performed, or clinical focus
                areas.
              </p>
              <TagInput
                tags={expertise}
                ...
                onRemove={(i) =>
                  setExpertise((x) => x.filter((_, j) => j !== i))
                }
        ...
        <section className="dp-card">
          ...
          {editing ? (
            <>
              <p className="dp-hint">
                Languages you can consult in with patients.
              </p>
              <TagInput
                tags={languages}
                onAdd={(t) => setLanguages((l) => [...l, t])}
                onRemove={(i) =>
                  setLanguages((l) => l.filter((_, j) => j !== i))
                }
        ...
        <section className="dp-card">
          ...
          {editing ? (
            <>
              <p className="dp-hint">
                List peer-reviewed papers, journal articles, or notable research
                contributions.
              </p>
                  ...
                  <div className="dp-entry-fields dp-grid-2">
                    <Field label="Title">
                      <input
                        ...
                        onChange={(e) =>
                          updatePub(p.id, "title", e.target.value)
                        }
                      />
                    </Field>
                    <Field label="Journal / Conference">
                      <input
                        ...
                        onChange={(e) =>
                          updatePub(p.id, "journal", e.target.value)
                        }
                      />
                    </Field>
                    <Field label="Year">
                      <input
                        ...
                        onChange={(e) =>
                          updatePub(p.id, "year", e.target.value)
                        }
                      />
                    </Field>
                  </div>
        ...
        <section className="dp-card">
          ...
          {editing ? (
            <>
              <p className="dp-hint">
                Set your available days and consultation hours.
              </p>
              <div className="dp-availability">
                {availability.map((d, i) => (
                  <div
                    key={d.day}
                    className={`dp-day-row ${d.available ? "dp-day-active" : ""}`}
                  >
            ...
            <>
                  ...
                  >
                    <span
                      ...
                      style={{
                        ...
                        color: d.available ? "#111827" : "#9ca3af",
                      }}
                    >
                      {d.available ? "✓" : "✗"} {d.day}
                    </span>
              ...
              <div className="dp-avail-extras" style={{ marginTop: 12 }}>
                <p
                  style={{
                    margin: "4px 0",
                    color: emergency.available ? "#166534" : "#6b7280",
                  }}
                >
                ...
                <p
                  style={{
                    margin: "4px 0",
                    color: emergency.telehealth ? "#166534" : "#6b7280",
                  }}
                >
                  {emergency.telehealth ? "✓" : "✗"} Telehealth / online
                  consultations
                </p>
                {emergency.maxPatients && (
                  <p style={{ margin: "4px 0", color: "#374151" }}>
                    Max patients per day:{" "}
                    <strong>{emergency.maxPatients}</strong>
                  </p>
                )}
              </div>
            </>
      ...
      {editing && (
        <div className="dp-footer">
          <p className="dp-footer-note">
            Changes are saved to your profile and visible to clinic
            administrators.
          </p>
