    ...
    Plotly.react(
      ...
      ],
      {
        title: {
          ...
          font: { size: 18, color: "#1e293b", family: "DM Sans, sans-serif" },
        },
        xaxis: {
          title: {
            text: "Measurement Date",
    ...
    <div className="chdr-page">
      ...
      <div className="chdr-stats">
        <div className="stat-card">
          ...
          <small className="stat-card__sub">
      ...
      {allergies.length > 0 && (
        <div className="alert-box">
          <div className="alert-box__header">
            ...
            <span className="alert-box__count">
      ...
      <div className="chdr-grid">
          ...
          {latestGrowth ? (
            <div className="growth-metric-grid">
              <div className="growth-metric-box">
                <span className="growth-metric-box__value">
                  {latestGrowth.weight ?? "—"}
                </span>
                ...
                {growthHistory.map((g, i) => (
                  <li key={i} className="growth-history__row">
                    ...
                    <span className="growth-history__row-values">
                      <span>
                        <strong>{g.weight ?? "—"}</strong> kg
                      </span>
                      <span>
                        <strong>{g.height ?? "—"}</strong> cm
                      </span>
                      <span>
                        <strong>{g.head ?? "—"}</strong> cm
                      </span>
                    </span>
                  </li>
                ))}
        ...
        <div className="chdrcard">
          <div className="chdrcard__header">
            ...
            <span className="chdrcard__badge">
              {vaccines.length} of latest 5
            </span>
          </div>
              ...
              {vaccines.map((v, i) => (
                <li key={i} className="vaccine-row">
                  <span className="vaccine-row__name">
                    {v.vaccine_name}
                    {v.dose_number && (
                      <span className="vaccine-row__dose">
                        Dose {v.dose_number}
                      </span>
                    )}
                  </span>
                  <span className="vaccine-row__right">
                    ...
                    <span
                      className={`status-badge status-badge--${v.status === "completed" ? "active" : "pending"}`}
                    >
                      {v.status || "—"}
                    </span>
                  </span>
                ...
                <li key={i} className="note-item">
                  <div className="note-item__top">
                    <span className="note-item__title">
                      {h.title || h.record_type}
                    </span>
                    {h.severity && (
                      <span
                        className={`status-badge status-badge--${getSeverityClass(h.severity)}`}
                      >
                        {h.severity}
                      </span>
                    )}
                  ...
                  <div className="note-item__sub">
                    ...
                    <span className="note-item__date">
                      {formatDate(h.record_date)}
                    </span>
                  </div>
                  {(h.temperature || h.description) && (
                    <div className="note-item__body">
                      {h.temperature && (
                        <span className="note-item__temp">
                          🌡 {h.temperature}°C
                        </span>
                      )}
                      {h.description && (
              ...
              <tbody>
                {(selectedChild.vaccinations || []).map((v, i) => (
                  <tr key={i}>
                    ...
                    <td>
                      <span className="nurse-table__vaccine-name">
                        {v.vaccine_name}
                      </span>
                      {v.dose_number && (
                        <span className="nurse-table__dose">
                          Dose {v.dose_number}
                        </span>
                      )}
                    </td>
                    ...
                    <td>
                      <span
                        className={`nurse-badge nurse-badge--${v.status === "completed" ? "completed" : "pending"}`}
                      >
                        {v.status || "—"}
                      </span>
                    </td>
